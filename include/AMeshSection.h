...
protected:
	...
	, mDefaultTransformation( new glm::mat4( source->getDefaultTransformation() ) )
	{
        ...
        const int _aiTextureType_DIFFUSE = 0x1;
        if (textureSurfaces.count(_aiTextureType_DIFFUSE)) {
			
            mTexture = ci::gl::Texture2d::create(*textureSurfaces.at(_aiTextureType_DIFFUSE),
